...
import {walletHeaders, walletMgtData, walletMgtHeaders} from "@/data/tableData";
import WalletThresholdModal from "@/modals/wallet-management/WalletThresholdModal";
import {useDispatch, useSelector} from "react-redux";
import {getWalletData, getWithdrawalRequest} from "@/features/wallet/wallet.slice";
...
function WalletMgtPage({}) {
    ...
    const { withdrawalRequests } = useSelector((state: RootState) => state.wallet as { withdrawalRequests: {
            id: string; txn_id: string; fullname: string; amount: number; created_at: string; status: string }[] }) || { withdrawalRequests: null };
                    ...
                    <div className={"border-[1px] border-grey-20 rounded-[12px] flex flex-col"}>
                        ...
                        <div className="bg-white shadow-md rounded-lg">
                            <table className="min-w-full table-auto border-collapse">
                                ...
                                <tbody>
                                {
                                    withdrawalRequests && withdrawalRequests.length > 0 ? withdrawalRequests.map((row, index) => (
                                        <tr key={index} className="border-b border-grey-20 h-[72px] cursor-pointer" onClick={() => window.location.href = `/wallet-management/${row.id}`}>
