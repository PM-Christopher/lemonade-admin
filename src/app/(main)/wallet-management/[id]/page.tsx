...
function WalletDetailsPage({}) {
    ...
    const { walletDetail, loading: walletLoading } = useSelector((state: RootState) => state.wallet) as { walletDetail: { status?: string } | null, loading: boolean };
        ...
        <MainLayout>
            <section className={"p-[20px] flex justify-between"}>
                <div className={"w-[600px] h-fit bg-white p-[24px] flex flex-col gap-[20px] rounded-[12px]"}>
                    ...
                    {
                        walletDetail && walletDetail.status === "pending" && (
                            <div className={"flex justify-between gap-[16px]"}>
                                <button
                                    className={"border-[1px] border-light-grey-50 px-[48px] py-[11px] rounded-[12px] bg-white w-full"} onClick={toggleWithdrawalReject}>
                                    <p className={"text-black text-[16px] font-medium"}>Reject withdrawal</p>
                                </button>
                                <button
                                    className={"border-[1px] border-step-color px-[48px] py-[11px] rounded-[12px] bg-gradient-green w-full"} onClick={toggleWithdrawalAction}>
                ...
                <div className={"h-[762px] bg-white rounded-[12px]"} style={{width: "908px"}}>
                    ...
                    <div className={"pt-[24px] px-[24px] pb-[12px]"}>
                        <div
                            className={"flex border-[1px] border-light-grey-50 bg-none w-fit h-[44px] px-[14px] py-[12px] rounded-[12px] gap-[8px] items-center cursor-pointer"} onClick={toggleUpdateBalance}>
