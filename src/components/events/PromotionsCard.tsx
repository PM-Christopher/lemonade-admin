        ...
        <div className={'flex flex-col gap-[16px] p-[24px] bg-white rounded-[12px]'}>
            <div className={'flex justify-between'}>
                ...
                <div className={'flex gap-[4px]'}>
                    ...
                    <TrashIcon className={'text-red-1 cursor-pointer'} onClick={() => handleDeletePromotion(promotion?.id)}/>
                </div>
            </div>
            ...
            <div className={'flex flex-col gap-[8px]'}>
                {
                    promotion?.breakdown?.map((item: any, index: number) => (
                        <p key={index} className={'text-[14px] font-normal text-light-black'}>{item}</p>
                    ))
                }
            </div>
        </div>
