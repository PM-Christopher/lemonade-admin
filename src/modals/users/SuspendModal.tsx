...
function SuspendModal({isOpen, toggle, id}: SuspendModalProps) {
    ...
    const SubmitAction = () => {
        if (authToken && id) {
            dispatch(userAction({token: authToken, id, actionType: "suspend"})).then((res: any) => {
                if (res.status === 200) {
                    toggle();
                }
            })
            ...
            <div className="w-[360px] bg-white pt-[16px] pb-[4px] rounded-[12px]">
                ...
                <div className={"py-[16px] flex flex-col gap-[16px] px-[16px]"}>
                    <p className={"text-[14px] font-normal text-light-black"}>
                        Are you sure you want to suspend this user? They will no longer have access to their account and other account activities.
                    </p>
                    ...
                    <div className={"flex justify-between gap-[10px]"}>
                        ...
                        <button className={"h-[48px] border-[1px] bg-red-1 border-red-2 py-[14px] rounded-[12px] w-[156px]"} onClick={SubmitAction}>
